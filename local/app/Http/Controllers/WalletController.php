...
class WalletController extends Controller
{
	...
	public function sangvish_showpage() {
		...
		if(!empty($with_count))
		{
			...
			$queryy=DB::table('booking')
					->where('shop_id', '=', $shop_id)
					->where('status', '=', 'paid')
					->get();
					
			$get_amount=DB::table('withdraw')
					->where('withdraw_shop_id', '=', $shop_id)
					...
					{	
						 ...
						 $shoper_balance +=$balances->total_amt - $balances->admin_commission;
						 //$shop_balance +=$balances->total_balance - $balances->withdraw_amt;
					}
					
					$get_balance="";
                    foreach($get_amount as $gamount)
					{	
						
						 $get_balance +=$gamount->withdraw_amt;
					}
                    					
			       $shop_balance = $shoper_balance - $get_balance;
	    ...
	    if(empty($with_count))
		{
			$nquery=DB::table('booking')
					...
					{
						$bal +=$nbalance->total_amt - $nbalance->admin_commission;
  ...
  public function sangvish_savedata(Request $request)
   {
	  ...
	  $total_bal = $data['shop_balance'];
	   ...
	   if($min_with_amt<=$withdraw_amt && $shop_balance>=$withdraw_amt)
		{
			DB::insert('insert into withdraw (shop_balance,withdraw_amt,total_balance,withdraw_mode,paypal_id,stripe_id,payumoney,bank_acc_no,bank_info,ifsc_code,withdraw_shop_id,withdraw_status
			) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [$shop_balance,$withdraw_amt,$total_bal,$withdraw_mode,$paypal_id,$stripe_id,$payumoney,$bank_acc_no,$bank_name,$ifsc_code,$shop_id,$with_status]);
